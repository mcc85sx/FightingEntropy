Function Get-ViperBomb
{
    ...
    $UI    = [XamlWindow]::New($Viper.Xaml)
        ...
        $UI.Host.Title = ("{0}v{1}" -f $This.Config.Name, $This.Config.Version )
        ...
        If ( $UI.Host.LogSvcSwitch.IsChecked -eq 0 ) { $UI.Host.LogSvcSwitch.Browse.IsEnabled = 0 }
        If ( $UI.Host.LogSvcSwitch.IsChecked -eq 1 ) { $UI.Host.LogSvcSwitch.Browse.IsEnabled = 1 }

        # LogScrSwitch / LogScrBrowse / LogScrLabel
             $UI.Host.LogScrSwitch.IsChecked   = 0 ;   $UI.Host.LogScrBrowse.IsEnabled        = 0
        If ( $UI.Host.LogScrSwitch.IsChecked -eq 1 ) { $UI.Host.LogScrSwitch.Browse.IsEnabled = 1 }
        If ( $UI.Host.LogScrSwitch.IsChecked -eq 0 ) { $UI.Host.LogScrSwitch.Browse.IsEnabled = 0 }
        ...
        $UI.Host.LogSvcLabel.Text              = "<Activate to designate a different file name/path>" 
        $UI.Host.LogScrLabel.Text              = "<Activate to designate a different file name/path>" 
        $UI.Host.RegLabel.Text                 = "<Activate to designate a different file name/path>" 
        $UI.Host.CsvLabel.Text                 = "<Activate to designate a different file name/path>" 
        ...
        $UI.Host.ServiceConfigLabel.Content    = $UI.Host.ServiceConfig.SelectedItem.Content
       
        # ScriptConfig /  ScriptConfigLabel
        $UI.Host.ScriptConfigLabel.Content     = $UI.Host.ScriptConfig.SelectedItem.Content
