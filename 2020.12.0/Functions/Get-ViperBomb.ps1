Function Get-ViperBomb
{
    ...
    $Info        = [_Info]::New()
    $VB          = [_ViperBomb]::New()
    $Service     = (Get-FEService)
    $UI          = (Get-XamlWindow -Type FEService)
    ...
    $UI.Host.About.Add_Click(
        {
            [System.Windows.MessageBox]::New(($VB.About -join "`n"),"About")
        }
    )
    $UI.Host.Copyright.Add_Click(
        {
            [System.Windows.MessageBox]::New(($VB.Copyright -join "`n"),"Copyright")
        }
    )
    ...
    { 
        [System.Windows.MessageBox]::New(($VB.Help -join "`n"),"Help")
    })
