Function New-FEADObject
{
    [CmdletBinding()]
    Param(
    [Parameter(Mandatory)][Object]$Key,
    [Parameter(Mandatory)][String]$OUName,
    [Parameter(Mandatory)][String]$OUDescription,
    [Parameter(Mandatory)][String]$SGName,
    [Parameter(Mandatory)][String]$SGDescription)
    ...
    $PG = @(("Administrators","Builtin"),("Enterprise Admins","Users"),("Domain Admins","Users")) | % {
        @{    
            Identity     = $SGDN
            MemberOf     = "CN={0},CN={1},$DN" -f $_[0,1]
        }
    }
    ...
    If (!( Get-ADObject -Filter * | ? ObjectClass -match Unit | ? Name -eq $OUName ))
    {
        New-ADOrganizationalUnit @OrgUnit -Verbose
    }

    If (!( Get-ADObject -Filter * | ? ObjectClass -match Group | ? Name -eq $SGName ))
    {
        New-ADGroup @SecGroup -Verbose
        $PG | % { Add-ADPrincipalGroupMembership @_ -Verbose }
    }
    ...
    $UI.IO._Ok.Add_Click({
        ...
        Else
        {
            ...
            If ( $Member.UserPrincipalName -notin $UI.Users.UserPrincipalName )
            {
                ...
                If ( $User )
                {
                    Write-Theme ("Success [+] {0}" -f $User.UserPrincipalName)
                    $UI.IO.DialogResult   = $True
                }
