...
#{
    ...
    Class VMGateway
    {
        ...
        Hidden [String] $VMX
        Hidden [String] $VHDX
        VMGateway([Object]$Item,[String]$Mem,[String]$HD,[UInt32]$Gen,[String]$Switch)
        {
            $This.Item               = $Item
            $This.Name               = $Item.Name
            $This.MemoryStartupBytes = "$Mem`MB"
            $This.Path               = "{0}\$($Item.Name).vmx"
            $This.NewVhdPath         = "{0}\$($Item.Name).vhdx"
            $This.NewVhdSizeBytes    = "$HD`GB"
            $This.Generation         = $Gen
    ...
    Class VMSilo
    {
        ...
        VMSilo([String]$Name,[Object[]]$Gateway)
        {
            ...
            {        
                $This.Gateway += [VMGateway]::New($Item,1024,20,1,$This.External.Name)
            }

            $This.Gateway      | % { 

                $_.Path        = $_.Path       -f $This.VMHost.VirtualMachinePath
                $_.NewVHDPath  = $_.NewVHDPath -f $This.VMHost.VirtualHardDiskPath
            }
        }
    }
...
# Add-Type -AssemblyName PresentationFramework
# ( GC $Home\Desktop\New-FEDeploymentShare.ps1 ) -join "`n" | IEX
...
$Gateway   = $Main.Gateway | ? Name -ne CP-NY-US-12065
