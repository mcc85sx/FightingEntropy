...
Function Get-PSDLocalDataPath
{
    ...
    If ($LocalPath -eq "")
    {
        ...
        Get-Volume | ? {-not [String]::IsNullOrWhiteSpace($_.DriveLetter) } | ? DriveType -eq Fixed | ? DriveLetter -ne X | ? {Test-Path "$($_.DriveLetter):\MININT"} | Select-Object -First 1 | % {
            $LocalPath = "$($_.DriveLetter):\MININT"
        }
    }
...
Function Set-PSDCommandWindowsSize
{
    ...
    #>
    [CmdletBinding()]
    Param([Parameter(Position=0)][UInt32]$Height=40,
          [Parameter(Position=1)][UInt32]$Width=120)
